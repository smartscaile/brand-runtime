---
name: presentation
description: Create, restructure, refine, review, export, and package presentation decks using Brand Runtime identity authority and a deterministic HTML-to-PDF workflow. Use when the Presentation command or Brand presentation modifier is invoked, or for presentation, slide, slide deck, pitch deck, keynote, strategy deck, HTML deck, PDF deck, PowerPoint, PPTX, Google Slides, slide-by-slide refinement, visual hierarchy, copy comparison, presentation QA, or export repair.
---

# Presentation

Direct presentation work as a specialized surface of Brand Runtime. Own narrative structure, slide composition, iterative refinement, fixed-page HTML mechanics, PDF export, and rendered QA. Never own or invent client identity.
...
- Preserve the requested output format. Prefer fixed-page HTML plus a validated PDF when browser sharing, exact rendering, navigation, or direct download matters. Use the available native presentation workflow when editable PPTX or Google Slides is the real requirement.
...
4. **Export or repair:** produce and verify final HTML, PDF, PPTX, or Google Slides artifacts.
...
- Include screen-only Previous, Next, page count, and Download PDF controls.
- Make keyboard navigation deterministic.
- Download the finalized, validated PDF bytes directly; never regenerate the PDF in the recipient's browser.
...
- Embed only the final validated PDF in the shareable HTML.

Read `references/html-delivery.md` before implementing or exporting HTML. Use `scripts/presentation-runtime.mjs` for deterministic scaffold, export, packaging, and baseline QA.
...
- no Type 3 PDF fonts;
- no browser print instructions as the recipient workflow.
...
Block delivery until all applicable checks pass:
...
- PDF page count and dimensions match the deck;
- fonts are embedded and no unexpected Type 3 font exists;
- the downloaded PDF hash matches the validated PDF;
...
Read `references/qa.md` for the complete rendered validation sequence.

## Commands

```bash
node <skill-dir>/scripts/presentation-runtime.mjs scaffold --output <project-dir> --title <title>
node <skill-dir>/scripts/presentation-runtime.mjs export --input <authoring.html> --pdf <deck.pdf> --html <shareable.html> --qa-dir <qa-dir>
...
- Read `references/html-delivery.md` for fixed-page HTML, controls, PDF download, self-contained packaging, or renderer-safe implementation.
