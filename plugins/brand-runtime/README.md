...
Project start treats the host workspace and `--project` as discovery hints. It proposes a concrete project name and absolute path, requires user confirmation, and only then audits that project's existing instructions, stack, content, and design documentation. It does not require a particular client folder layout.
